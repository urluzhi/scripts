...
my $svm_scaler = "$dir"."svm/svm_scaler";
my $svm_model ="$dir"."svm/svm_model";
my $svm_scale ="$dir"."svm/svm-scale";
my $svm_predict ="$dir"."svm/svm-predict";
my $OligoWalk_exe="$dir"."exe/OligoWalk";
my $NN_data="$dir"."NN_data/";
	...
	my ($file)= @_;
		...
		foreach my $outline(@outdata) {
			if($outline=~/<\/table>/) {#check if line hit the end of energy table
				...
				if(!$energies[8]) { $energies[8]="0.00";}
				if(!$energies[9]) { $energies[9]="0.00";}
...
#		push @score,$DH_ALL/$DS_ALL;
		...
		$end_diff[$i]=&DG($base[1-1],$base[2-1]) - &DG($base[18-1],$base[19-1]);
