...
int SHAPEend(int i, structure *ct) {
	if (ct->shaped) if (ct->SHAPE[i]>-500) return (int) (log(ct->SHAPE[i]+1.0)*ct->SHAPEslope+ct->SHAPEintercept);
	return 0;
}
	...
	char a, char b)
{
	...
	*/
      ...
      if ((a>0)||(b>0)) {
      	if ((a&DOUBLE)||(b&DOUBLE)) return INFINITE_ENERGY;//the loop contains a nuc that
...
int erg2in(int i,int j,int ip,int jp,structure *ct, datatable *data)
{
	...
	int energy,size,size1,size2,lopsid;
	/* size,size1,size2 = size of a loop
		energy = energy calculated
		loginc = the value of a log used in large hairpin loops
	*/
...
int energy,size,loginc,count,key,k;
	...
	*/
		...
		if (dbl&DOUBLE) return INFINITE_ENERGY;//the loop contains a base that should be
      										//double stranded
...
//Correct the free energy for a structure that has SHAPE mapping.
...
int correctshape(int i,int j,structure *ct) {
	int energy;
	energy=0;


	//current version needs no correction because SHAPE is applied twice per base pair
	//if (ct->shaped) {
	//	if (ct->SHAPE[i]>-500) energy-=(int) (ct->SHAPE[i]*ct->SHAPEslope+ct->SHAPEintercept);
	//	if (ct->SHAPE[j]>-500) energy-=(int) (ct->SHAPE[j]*ct->SHAPEslope+ct->SHAPEintercept);
