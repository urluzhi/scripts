	...
	//option 2: refold  rna
		...
		if (foldsize ==0) {
			...
			//option 2 +notrefold+ partionfunction
				...
				target->refill(ct,&Qc,i, i+length-1,rescaleinrefill);
				table[i][2] = (int)(conversionfactor*rt* 
					( (long double)(ct->numofbases)*logl(rescaleinrefill) 
					+ logl((long double)Q/(long double)Qc) ) );
				cout<< "DEBUG "<<i<<"\tQc  "<<Qc<<"\t Q: "<<Q<<"\tRESCALE: "<<rescaleinrefill<<"\t";
				cout<<"Breaking: "<<table[i][2]<<"\t scale: "<<pfdata->scaling<<"\t";
				cout<<"bases: "<<ct->numofbases<<"\n";
		...
		//scan: only folding the region close to the binding site
			...
			//option 2 + refold + partionfunction
				...
				target->scanconstrain(&fracct,&Qc,foldstart,foldstop,rescaleinrefill);
...
//--------------------------------------------------------------------------------------------------------  
	...
	if (option==1) {
		...
		delete[] temp;
				...
				cout<<"\n";
		if (Usesub==1 && foldsize==0 )	delete[] energyarray;
		else if (Usesub==2) {
