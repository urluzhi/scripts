...
void siprefilter::count(structure *ct,int i,int length,int test) {
	...
	DG3=0;
	for (j=i;j<=i;j++)	DG3+=stack[ct->numseq[j]][ct->numseq[j+1]];
	
	DG3+=end[ct->numseq[i]];
	//substract the AU penalty if the 5th begin with AU
	DG3-=end[ct->numseq[i+1]];
	//5' end of the antisense strand,3' end of the target sequence
	DG5=0;
	for (j=i+length-2;j<=i+length-2;j++) DG5+=stack[ct->numseq[j]][ct->numseq[j+1]];
	DG5+=end[ct->numseq[i+length-1]];
	//substract the AU penalty if the 5th begin with AU
	DG5-=end[ct->numseq[i+length-2]];
...
Usesub 1 - using suboptimal structures for self-struture of target RNA (oligo free)
Usesub 2 - using partition funcion considering every possible structure  of target
			   suboptimal 2 can only used with mold 2
Usesub 3 - using suboptimal structures for both oligo-free and oligo-bound target RNA
...
void olig(bool isdna, int option, structure *ct, int length,double c, int **table,int **numofsubstructures,
		  ...
		  thermo* helixstack,int start, int stop, siprefilter *prefilter,int foldsize,int *TEST) {
	...
	int dh,ds,dgeff;
	int *energyarray;
	int numofstructures,tempnumofstructures;
	long double k1b,k1u;
	...
	//option 2: refold  rna
		...
		if (foldsize ==0) {
			...
			//option 2 +notrefold+ consider all suboptimal structures
				...
				strcpy(savefile, ct->ctlabel[1]);
				savefile[strlen(savefile)-1]='\0'; 
				sprintf(pos,"_%d_s1_0_constrain.sav",i);
				strcat(savefile,pos);
				ifstream sav(savefile,ios::binary);
				if (TESTMODE && sav) {
					//read information from file if it was found
					read(&sav,&tempnumofstructures);
					ct->numofstructures=1;
					read(&sav, &(ct->energy[1]));
					sav.close();
				}
				else {
					sav.close();
			...
			//option 2 +notrefold+ consider all suboptimal structures with average ensemble
			else if (Usesub==3) {
				sum = 0;
				sn = 0;
				for (k=1;k<=numofstructures;k++) {
				...
				sprintf(pos,"_%d",i);
				strcat(savefile,pos);
				strcat(savefile,"_s1_0_constrain.sav");
				ofstream sav(savefile,ios::binary);
		...
		//scan: only folding the region close to the binding site
			...
			//option 2 + refold + consider all suboptimal structures
				...
				strcpy(savefile, ct->ctlabel[1]);
				savefile[strlen(savefile)-1]='\0'; 
				sprintf(pos,"_%d_s1_%d_constrain.sav",i,foldsize+length);
				strcat(savefile,pos);
				ifstream sav(savefile,ios::binary);
				if (TESTMODE && sav) {
					//read information from file if it was found
					read(&sav,&tempnumofstructures);
					fracct.numofstructures=1;
					read(&sav, &(fracct.energy[1]));
					sav.close();
				}
				else {
					sav.close();
			...
			//---------------------------------------------------------------------------------------------
			//option 2 + refold + consider all suboptimal structures average ensemble
			else if (Usesub==3) {
      			sum = 0;
				sn = 0;
				for (k=1;k<=numofstructures;k++) {
...
void report(char* filename, structure *ct, int **table,int **numofsubstructures, int length, bool isdna,
