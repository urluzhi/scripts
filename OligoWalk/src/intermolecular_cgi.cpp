...
void olig(bool isdna, int option, structure *ct, int length,double c, int **table,datatable& data,
	...
	short int dh,ds,dgeff;
	short int *energyarray;
	int numofstructures;
	long double k1b,k1u;
	...
	if (option ==1) {
		if (Usesub==0)	{
			...
			efn2(&data,ct);
			energyarray = new short int [ct->numofstructures+1];
		...
		if (Usesub ==0 ) {
			...
			energyarray = new short int [1000+1];
			if (foldsize ==0) {	
				ct->nnopair=0;
				dynamic(ct,&data,1000,10,0);
	...
	if (option==1) {
		...
		if (Usesub==1) { 
            ...
            for (k=1;k<=ct->numofstructures;k++) {
	          	...
	          	fn = expl(-((energyarray[k])/(rt*conversionfactor)));
                sn = sn + fn;
	...
	//option 2: refold  rna
		...
		//scan: only folding the region close to the binding site
			...
			if ( (i-foldsize/2) > start && (i+length-1+foldsize/2) < ct->numofbases ) {
			...
			if( (i-1+length+foldsize/2)==ct->numofbases )
			for (j=1;j<=foldsize+length;j++) {
      			fracct.numseq[j] = ct->numseq[stop+j-foldsize-1];
			}
			fracct.nnopair=0;
			...
			if (Usesub ==0) {
				...
				if (prefilter->useit == 0) {
					...
					else if((i-foldsize/2)>start && (i+foldsize/2)<=stop){
					...
					if ( (i-foldsize/2)<start || (i+foldsize/2)>=stop ) {
				...
				//using prefilter, not reuse arrays,refold the new region every time
					...
					//define the sequence again, since not knowing folding start point when prefilter is used
					if (i-foldsize/2 <= start) 
						for (j=1;j<=foldsize+length;j++) {
      						fracct.numseq[j] = ct->numseq[j];
						}
					if(	(i+length-1+foldsize/2) > ct->numofbases  )
						for (j=1;j<=foldsize+length;j++) {
      						fracct.numseq[j] = ct->numseq[stop+j-foldsize-1];
						}
			...
			fracct.checknopair();
			if (i-foldsize/2<=start) {//constrained positions are different at two ends, as the folded region did not move
				for (j=0;j<length;j++) {
			...
			else if(i+foldsize/2>=stop) {
				for (j=0;j<length;j++) {
					fracct.nopair[j+1] = foldsize+1-stop+i+j;
				}
				foldstart=foldsize+1-stop+i;
				foldstop=foldsize+length-stop+i;
			}
    ...
    if ((k1u/(c*k1b))<100) {
      	...
      	dgeff = (short)(-((1.9872)*(3.1)*logl(((4*k1b*(long double)(c))/(-1-k1u+sqrtl(powl(1+k1u,2)
