		...
		if (Usesub ==0 ) {
			...
			if (foldsize ==0) {	
				...
				if ((!sav) || 1) { //must recalculate
					...
					cout << "Number of suboptimal structures: -> "<<numofstructures<<"  <br>\n";
			...
			if (foldsize==0) {//folding the whole sequence at one time
				...
				if ((!sav) || 1) { //must write
					...
					strcat(savefile,"_sample");
					target->partition4refill(&Q,savefile);
					stochastic(ct,savefile,1000,5);
					efn2(&data,ct);
	...
	//option 2: refold  rna
		...
		if (foldsize ==0) {
			...
			//option 2 +notrefold+ consider all suboptimal structures
				...
				cout << "Number of constrained suboptimal structures : -> "<<ct->numofstructures<<"  <br>\n";
				cout<<"\n";
			...
			//option 2 +notrefold+ partionfunction
				...
				target->refill(ct,&Qc,i, i+length-1,rescaleinrefill,savefile);
				stochastic(ct,savefile,1000,5);
				efn2(&data,ct);
				sum = 0;
				sn = 0;
		...
		//scan: only folding the region close to the binding site
			...
			if (Usesub ==0) {
				...
				sprintf(savefile,"%d_%d_",foldsize+length,i);
				strcat(savefile, ct->ctlabel[1]);
				savefile[strlen(savefile)-1]='\0'; //get off the new line charactor in the string
				strcat(savefile,".pfs");
				...
				if (prefilter->useit == 0) {
					if (i==start) {
						...
						target->partition(true,&Q,NULL,savefile);
					...
					target->scanfill(&fracct,&Q,0,savefile);
					}
				...
				//using prefilter, not reuse arrays,refold the new region every time
					...
					target->partition(true,&Q,NULL,savefile);
					stochastic(&fracct,savefile,1000,5);
					efn2(&data,&fracct);
			...
			//option 2 + refold + consider all suboptimal structures
				...
				cout<<"\n";
				cout << "Number of suboptimal structures: -> "<<fracct.numofstructures<<"  <br>\n";
				table[i][2] = (int)(sum/sn) - table[i][2];
			...
			//option 2 + refold + partionfunction
				...
				target->scanconstrain(&fracct,&Qc,foldstart,foldstop,rescaleinrefill,savefile);
				stochastic(&fracct,savefile,1000,5);
				efn2(&data,&fracct);
				sum = 0;
				sn = 0;
...
//--------------------------------------------------------------------------------------------------------  
	...
	if (option==1) {
		...
		delete[] temp;
				...
				cout<<"\n";
		if (Usesub!=0 && foldsize==0 )	delete[] energyarray;
		else if (Usesub==2) {
